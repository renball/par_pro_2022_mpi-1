...
int par_summa(std::vector<int> vect, int size) {
    ...
    if (ProcRank == 0) {
        cur_vec = std::vector<int>(vect.begin(), vect.begin() + d);
        for (size_t i = 1; i < t; i++) {
            MPI_Send(vect.data() + i * d, d, MPI_INT, i, 0, MPI_COMM_WORLD);
        }
    } 
