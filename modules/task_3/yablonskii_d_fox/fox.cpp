...
namespace {
    ...
    void makeGRc(int grs, int procRank, int* grc) {
        ...
        MPI_Cart_create(MPI_COMM_WORLD, 2, ds.data(), t_per.data(), 0, &G_COM);
        MPI_Cart_coords(G_COM, procRank, 2, grc);
        
        sbd[0] = 0; sbd[1] = 1;
        MPI_Cart_sub(G_COM, sbd.data(), &R_COM);

        sbd[0] = 1; sbd[1] = 0;
        MPI_Cart_sub(G_COM, sbd.data(), &C_COM);
    }
