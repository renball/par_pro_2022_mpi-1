...
namespace {
    ...
    void makeGRc(int grs, int procRank, int* grc) {
        std::vector<int> ds(2, grs);
        std::vector<int> t_per(2, 0);
        std::vector<int> sbd(2);
        MPI_Cart_create(MPI_COMM_WORLD, 2, ds.data(), t_per.data(), 0, &GRID_COMM);
        MPI_Cart_coords(GRID_COMM, procRank, 2, grc);
        sbd[0] = 0;
        sbd[1] = 1;
        MPI_Cart_sub(GRID_COMM, sbd.data(), &ROW_COMM);
        sbd[0] = 1;
        sbd[1] = 0;
        MPI_Cart_sub(GRID_COMM, sbd.data(), &COL_COMM);
    }
...
namespace myns {
    ...
    std::vector<double> sM(const std::vector<double>& a,
    ...
    std::vector<double> fM(std::vector<double> a,
                                std::vector<double> b,
                                size_t size) {
        ...
        MPI_Status status;
        int GRi_s = static_cast<int>(sqrt(proc_num));
        if (GRi_s * GRi_s != proc_num) {
            throw "Wrong number of processes";
        }
        std::vector<int> GRi_c(2);
        makeGRc(GRi_s, proc_rank, GRi_c.data());
        int Bl_s;
        if (proc_rank == 0) {
        ...
        if (proc_rank == 0) {
            for (int p = 1; p < proc_num; p++) {
                MPI_Send(a.data() + (p % GRi_s) * Bl_s + (p / GRi_s) * size * Bl_s,
                    1, block_mat, p, 0, GRID_COMM);
                MPI_Send(b.data() + (p % GRi_s) * Bl_s + (p / GRi_s) * size * Bl_s,
        ...
        if (proc_rank != 0) {
            ...
            for (int p = 1; p < proc_num; p++) {
                MPI_Recv(res.data() + (p % GRi_s) * Bl_s + (p / GRi_s) * size * Bl_s,
                    Bl_s * Bl_s, block_mat, p, 3, MPI_COMM_WORLD, &status);
            }
        }
