...
namespace myns {
    ...
    std::vector<double> fM(std::vector<double> a, std::vector<double> b, size_t size) {
        ...
        int GRi_s;
        GRi_s = static_cast<int>(sqrt(Num_pc));
        ...
        if (Rank_pc == 0) {
            for (int x = 1; x < Num_pc; x++) {
                MPI_Send(a.data() + (x % GRi_s) * Bl_s + (x / GRi_s) * size * Bl_s, 1, block_mat, x, 0, G_C);
                MPI_Send(b.data() + (x % GRi_s) * Bl_s + (x / GRi_s) * size * Bl_s, 1, block_mat, x, 1, G_C);
            }
        }
        else {
            MPI_Recv(bla.data(), Bl_s * Bl_s, MPI_DOUBLE, 0, 0, G_C, &status);
            MPI_Recv(blb.data(), Bl_s * Bl_s, MPI_DOUBLE, 0, 1, G_C, &status);
        }
        ...
        for (int i = 0; i < GRi_s; i++) {
            ...
            MPI_Bcast(tmpblockA.data(), Bl_s * Bl_s, MPI_DOUBLE, p_tt, R_C);
            ...
            if (GRi_c[0] == 0)
                P_p = GRi_s - 1;
            MPI_Sendrecv_replace(blb.data(), Bl_s * Bl_s, MPI_DOUBLE, P_p, 0, N_p, 0, C_C, &status);
        }
        ...
        if (Rank_pc != 0) {
            ...
            for (int x = 1; x < Num_pc; x++) {
                MPI_Recv(res.data() + (x % GRi_s) * Bl_s + (x / GRi_s) * size * Bl_s, Bl_s * Bl_s, block_mat, x, 3, MPI_COMM_WORLD, &status);
            }
        }
